...
In this chapter, I examine whether adding feedback loops to the genetic toggle switch increases its robustness to parameter fluctuations. To do this, I use \acrshort{abc} \acrshort{smc} to estimate the parameter values that allow the toggle switch model to behave like a switch.  I then study the effect that adding feedback loops has to the toggle switch bistability, and finally I use model selection to select the most robust switch model out of the ones considered.
...
$β {u} \ce{<=>[\ce{Kd_u}][\ce{Km_u}]}{u_β}$        & \multirow{2}{*}{dimerization}    \\
...
\begin{align}\label{eq:gard_ma_ode}
	...
	\frac{dgu\bullet v_γ}{dt} &= Kf_{u}gv v_{γ} - Kr_{u}gu\bullet v_γ\\\label{eq:gard_ma_ode_last}
...
\noindent This can now be substituted into the set of Equations~\ref{eq:gard_ma_ode}-\ref{eq:gard_ma_ode_last}. The second assumption that is made in this system is that the rate of formation and dissociation of the polymerised transcription factor is in steady state. This allows us to solve the system for $u$ in a similar way as shown above and substitute it in Equations~\ref{eq:gard_ma_ode}-\ref{eq:gard_ma_ode_last}. This results in a simplified model, with fewer differential equations and parameters.
...
The parameters chosen here for the phase space analysis are within the range suggested by~\textcite{Gardner:2000vha}. Parameters $a_1$ and $a_2$ are set to 10, and β, γ set to 2. A vector plot shows that the system has two steady states as shown in Figure~\ref{fig:Gard_CS}. Both states were found to be stable by examining the eigenvalues of the system at each steady state using Mathematica~\autocite{mathematica:2016}.


I further study the system by conducting a bifurcation analysis, where all parameters remain constant to the values shown above, and only one parameter ($a_1$) is varied. The bifurcation diagram, given in Figure~\ref{fig:Gard_CS}C shows a saddle-node bifurcation. We observe that by varying the parameter for the effective rate of synthesis of repressor 1 while all other parameters remain constant, the system is bistable when 7 \ge{} $a_1$ \le{} 17.
...
$\textrm{A} + \textrm{A} \stackrel{\textrm{dim}}{\longrightarrow}\textrm{A\textsubscript{2}}$        & \multirow{2}{*}{dimerization}    \\
$\textrm{B} + \textrm{B} \stackrel{\textrm{dim}}{\longrightarrow} \textrm{B\textsubscript{2}}$       &                                  \\
$\textrm{A\textsubscript{2}} \stackrel{\textrm{dim\_r}}{\longrightarrow}\textrm{A} + \textrm{A}$     & \multirow{2}{*}{monomerization}  \\
$\textrm{B\textsubscript{2}} \stackrel{\textrm{dim\_r}}{\longrightarrow}\textrm{B} + \textrm{B}$     &                                  \\
$\textrm{gA} + \textrm{B\textsubscript{2}} \stackrel{\textrm{rep}}{\longrightarrow}\textrm{B\textsubscript{2}gA}$    & \multirow{2}{*}{repression}      \\
$\textrm{gB} + \textrm{A\textsubscript{2}} \stackrel{\textrm{rep}}{\longrightarrow}\textrm{A\textsubscript{2}gB}$    &                                  \\
$\textrm{B\textsubscript{2}gA} \stackrel{\textrm{rep\_r}}{\longrightarrow}\textrm{B\textsubscript{2}} + \textrm{gA}$  & \multirow{2}{*}{dissociation}    \\
$\textrm{A\textsubscript{2}gB} \stackrel{\textrm{rep\_r}}{\longrightarrow}\textrm{A\textsubscript{2}} + \textrm{gB}$ &                                  \\
...
\label{alg:param_scan}
 \begin{algorithmic}[1]
	...
	\EndFor
	\State Plot phase plot of roots for A\textsubscript{2}, B\textsubscript{2}
	\EndFor
  \end{algorithmic} 
\end{algorithm}
...
\caption[Latin Hypercube sampling]{\label{fig:lhs}Latin hypercube sampling ensures that the whole space is sampled evenly. For the two species concerned, A\textsubscript{2} and B\textsubscript{2}, we assume uniform distributions, shown in grey. The joint space of the two distributions is divided into smaller equal parts and a random sample is drawn from within each subspace. Diagram adapted from~\textcite{Youssef:2013jv}.}
\end{center}
\end{figure*}

The roots of each particle for each initial condition were found using the Python package SciPy~\autocite{Scipy}. A phase plot was made for each particle, which consists of the last time point value of one dimer plotted against the other. The parameter scan uncovered the presence of bistable and monostable systems given a different set of parameter values. An example of the phase plots for each of the two types of stabilities found during the scan is shown in Figure~\ref{fig:stab exampl}.

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{../../chapters/chapterABCSysBio/images/phase_plots_param_scan.png}
\caption[Phase space examples of a monostable and a bistable switch]{\label{fig:stab exampl}An example of each type of switch found during the parameter scan. Each graph represents the steady state values of one dimer plotted against the other, from one parameter set and 100 initial conditions.}
\end{figure*}
...
Here I use ABC-SysBio to fit a model to the design objectives of a switch-like behaviour. I extend the model used in Section~\ref{sec:paramscan} by adding two inducers to the system, S and R. S removes the A homodimer (A\textsubscript{2}) from the system by binding to it thus removing the repression on gene B. R removes B\textsubscript{2} from the system with the same mechanism. These inducers represent the stimuli that will turn the switch ON and OFF in a biological setting. The equations shown in Table~\ref{tab:toglle-inducer-equs} are added to the existing set of equations for the mass action switch: 
...
\toprule
Equation                                                                                & Description                           \\ \midrule
$\textrm{S} + \textrm{A\textsubscript{2}} \stackrel{\textrm{rep\_dim}}{\longrightarrow}\textrm{SA\textsubscript{2}}$    & \multirow{2}{*}{Inducer repression}   \\
$\textrm{R} + \textrm{B\textsubscript{2}} \stackrel{\textrm{rep\_dim}}{\longrightarrow}\textrm{RB\textsubscript{2}}$    &                                       \\
$\textrm{SA\textsubscript{2}} \stackrel{\textrm{rep\_dim\_r}}{\longrightarrow}\textrm{S} + \textrm{A\textsubscript{2}}$ & \multirow{2}{*}{Inducer dissociation} \\
$\textrm{RB\textsubscript{2}} \stackrel{\textrm{rep\_dim\_r}}{\longrightarrow}\textrm{R} + \textrm{B\textsubscript{2}}$ &                                       \\
...
%\end{array}
%$$
The range of parameter values shown to produce a bistable switch in Section~\ref{sec:paramscan} was used as priors and are shown in Table~\ref{tab:param_inf_params}. Initial conditions of the two genes, $gA$, $gB$ were set to 1, and species $A$ and $A_2$ were set to 50 and 10 respectively, in order to set the starting conditions of the switch to $B_2$ OFF. All other species initial conditions were set to 0. The system was simulated using \acrshort{ode}s.
...
\toprule
 ...
 (min\textsuperscript{-1})&(μM\textsuperscript{-1} min\textsuperscript{-1})&(min\textsuperscript{-1})&(μM\textsuperscript{-1} min\textsuperscript{-1})&(min\textsuperscript{-1})&(min\textsuperscript{-1})&(μM\textsuperscript{-1} min\textsuperscript{-1})&(min\textsuperscript{-1})&(min\textsuperscript{-1})&(min\textsuperscript{-1})\\
...
\begin{figure*}[tb]
	\begin{center}
\includegraphics[width=\textwidth]{../../chapters/chapterABCSysBio/images/toggle_switch_designs.pdf}
...
I used the PyDSTool~\autocite{Clewley:2012kj} in order to determine whether each of the 7 switches is capable of bistable behaviour. The same analysis as Section~\ref{sec:bifurc} was used to identify the steady states of the systems and their stabilities. As shown in Figure~\ref{fig:Gard_pos}, both single and double positive autoregulation are consistent with bistable behaviour. Two stable states were found for both cases when $k$ = 2 and δ = 1. 
...
ABC SMC model selection accounts for model complexity and parametric robustness in order to avoid the problem of overfitting. It automatically applies the rule of parsimony, or Occam's razor, which dictates the simplest model to account for the data is the best model~\autocite{Toni:2010}. This is done by penalising the addition of a parameter, corresponding to an increase in the volume of the prior if it does not result in a larger increase in the volume of the approximated posterior distribution~\autocite{Woods:2016eh}.  Bayesian model selection can be used to rank models according to how well they describe the data, or how likely they are to give rise to the data we wish to see in a system design setting.  
...
$\textrm{A\textsubscript{2}} + \textrm{gA} \stackrel{\textrm{aut\_1}}{\longrightarrow} \textrm{A\textsubscript{2}gA}$  & dimer self-association  \\
$\textrm{A\textsubscript{2}gA} \stackrel{\textrm{aut\_2}}{\longrightarrow} \textrm{A} + \textrm{A\textsubscript{2}gA}$ & self-induced expression \\
$\textrm{A\textsubscript{2}gA} \stackrel{\textrm{aut\_3}}{\longrightarrow} \textrm{A\textsubscript{2}}+ \textrm{gA}$   & dimer self-dissociation \\\midrule
\multicolumn{2}{c}{Positive autoregulation B}                                                                    \\\midrule
$\textrm{B\textsubscript{2}} + \textrm{gB} \stackrel{\textrm{aut\_1}}{\longrightarrow} \textrm{B\textsubscript{2}gB}$  & dimer self-association  \\
$\textrm{B\textsubscript{2}gB} \stackrel{\textrm{aut\_2}}{\longrightarrow} \textrm{B} + \textrm{B\textsubscript{2}gB}$ & self-induced expression \\
$\textrm{B\textsubscript{2}gB} \stackrel{\textrm{aut\_3}}{\longrightarrow} \textrm{B\textsubscript{2}}+ \textrm{gB}$   & dimer self-dissociation \\ \bottomrule
\end{tabular}
\end{table}
...
\toprule
 \textbf{ge}     & \textbf{rep}     & \textbf{rep\_r}     & \textbf{dim}    & \textbf{dim\_r}     & \textbf{deg}  & \textbf{rep\_dim}    & \textbf{rep\_dim\_r} & \textbf{deg\_sr}  & \textbf{deg\_dim}   & \textbf{aut\_1} & \textbf{aut\_2} & \textbf{aut\_3} \\
  (min\textsuperscript{-1})&(μM\textsuperscript{-1} min\textsuperscript{-1})&(min\textsuperscript{-1})&(μM\textsuperscript{-1} min\textsuperscript{-1})&(min\textsuperscript{-1})&(min\textsuperscript{-1})&(μM\textsuperscript{-1} min\textsuperscript{-1})&(min\textsuperscript{-1})&(min\textsuperscript{-1})&(min\textsuperscript{-1}) &(μM\textsuperscript{-1} min\textsuperscript{-1})&(min\textsuperscript{-1})&(min\textsuperscript{-1})\\
