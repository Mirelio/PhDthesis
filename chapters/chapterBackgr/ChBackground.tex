...
The data shown in Figure~\ref{fig:exampl_post} consists of 5000 random samples drawn from a three-dimensional multivariate normal distribution, of mean = 0 for all dimensions and variance σ
\begin{align*}
σ = \begin{bmatrix}
1 &1  &0 \\ 
 0& 1 &0 \\ 
...
\end{bmatrix}.
\end{align*}
\noindent The three-dimensional distribution is plotted as shown in Figure~\ref{fig:exampl_post}.

\begin{figure*}[h]
    \begin{center}
    \includegraphics[scale=0.43]{../../chapters/chapterBackgr/images/example_posterior.pdf}
    \caption[Example of a posterior distribution plot]{Visualising a three-dimensional distribution. A three-dimensional multivariate normal can be visualised by plotting the one-dimensional marginal distributions on the diagonal and the the two-dimensional marginal distributions are on the off-diagonal. Correlation can be visualised using the 2D marginal distributions.}
