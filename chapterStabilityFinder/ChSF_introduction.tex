...
\label{sec:rob_back}
During this thesis I use robustness in its parametric sense. I examine the robustness of a model to fluctuations in parameter values. The robustness of a model can be calculated by dividing the volume of its functional region by the volume of its priors. This is a measure of how big the posterior distribution is compared to the priors.   

For a given model design D and objective O we define the functional region $F$ as the region within the prior where O is satisfied. So within the prior we can assign 1 to any region that falls within $F$ and 0 to any region outside that. It comes from Bayes rule that:

\begin{align}
p(O|D_1) = \int p(O|\theta,D_1)p(\theta|D_1)d\theta,
\end{align}

\noindent where $p(x|\theta)$ is the likelihood, $p(\theta)$ is the prior, and $\displaystyle \int p(x|\theta)p(\theta)d\theta$ is the evidence. For a design with three parameters this becomes:
...
\noindent Therefore, we can use the ratio of the two robustness measures to calculate the Bayes factor. 
...
\begin{figure*}[htbp]
\begin{center}
\includegraphics[scale=0.7]{chapterStabilityFinder/images/MA-cs-dp-phase.png}
...
\begin{figure*}[htbp]
\begin{center}
\includegraphics[width=\textwidth]{chapterStabilityFinder/images/MA_sym_post.png}
...
The models used in the above analysis assume the parameters for equivalent reactions are equal. This is a constraint that simplifies the model. When building this model into a synthetic system in the lab, this assumption cannot be easily justified. When choosing promoters to build this synthetic system two promoters can be chosen to have similar strength but it will not necessarily be identical.  In order to study how this might affect the results, I further eliminate modelling assumptions made in the toggle switch by making the parameters representing gene expression ($ge$) and repression ($rep$), as well as the protein degradation parameters asymmetric (independent parameters for each protein, versus fixed to be equal). We find that the features of the posterior distributions of the symmetric and the asymmetric models remain the same. The posterior distribution of the asymmetric \acrshort{cs-ma} model was more constrained than the posterior distribution of the asymmetric \acrshort{dp-ma} model. 
...
In Section~\ref{sec:ma_sw} we found that the \acrshort{dp-ma} is more robust than the \acrshort{cs-ma} model. Here I want to test whether this result still remains when the priors of the models are made wider. I change the prior ranges of both models are measure their robustness each time. The results are shown in Figure~\ref{fig:priors_matter2}A. We find that the robustness measure changed significantly as we change the priors of the models. When both models have very narrow or when both models have wide priors then their robustness measures are very similar. When both models have vary narrow priors the Bayes factor is equal to 1.32 and when the priors are wide the Bayes factor is equal to 1.06. In both these cases there is no significant difference in robustness between the two models. When the priors for both models are narrow the Bayes factor is equal to 2.25. Most notably, when the priors for the \acrshort{cs-ma} model are very narrow and the priors of the \acrshort{dp-ma} model are narrow, the Bayes factor is at 


I further analyse the effect of the prior distribution on the robustness by varying the prior ranges of both models, \acrshort{cs-ma} and \acrshort{dp-ma}. Three sets of priors were used: very narrow priors are the priors used in Figure~\ref{fig:ma-sym-det-post}A, wide priors which are the priors used in Figure~\ref{fig:ma-sym-det-post}B and narrow priors which are intermediate between the two. The above sets of priors refer to the parameters the two models have in common which are set to be the same. Nevertheless this does not have to be the case so I also use priors where the common parameters do not have identical priors, but one is always a subset of the other. 


We see in Figure~\ref{fig:priors_matter2}A that the robustness for both 


\begin{figure*}[h]
\begin{center}
\includegraphics[width=\textwidth]{chapterStabilityFinder/images/priors_matter_2.png}
\caption[LoF caption]{ \label{fig:priors_matter2}:}
\end{center}
\end{figure*}
It is therefore important to note that when talking about the robustness of a model we must always refer to its robustness given a set of priors. This robustness measure cannot be generalised to the model, but it must always be referred to as the robustness of a model given these priors. 
